...
import {SafeAreaView, Dimensions, FlatList, Text, TextInput} from 'react-native'
  ...
  useEffect(()=>{
    if(searchQuery===""){
      ...
      const searchList = lista.filter((item)=>{
        return item.Ime.toLowerCase().indexOf(searchQuery.toLowerCase())!== -1 || item.Brand.toLowerCase().indexOf(searchQuery.toLowerCase())!== -1;
      });
      setLista(searchList)
    }
  },[searchQuery])
    ...
    <SafeAreaView style={{
      ...
      <Stack.Screen
        options={{
          ...
          headerTitle: (kategorija === null || kategorija === undefined) ? "Proizvodi" : kategorija 
        }}
      />
        <TextInput
